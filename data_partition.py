...
###
#目的是对大规模3D场景进行高斯分割，并根据相机视点对这些高斯分割进行遮罩处理。
###
def block_partitioning(cameras, gaussians, args, pp, scale=1.0, quiet=False, disable_inblock=False, simple_selection=False):
    """将高斯模型按照块进行分割，并根据相机视点对这些块进行遮

    Args:
        cameras (_type_): _description_
        gaussians (_type_): _description_
        args (_type_): _description_
        pp (_type_): _description_
        scale (float, optional): _description_. Defaults to 1.0.
        quiet (bool, optional): _description_. Defaults to False.
        disable_inblock (bool, optional): _description_. Defaults to False.
        simple_selection (bool, optional): _description_. Defaults to False.

    Returns:
        _type_: _description_
    """
    xyz_org = gaussians.get_xyz # 获取高斯的原始坐标
    num_threshold = args.num_threshold 
    block_num = args.block_dim[0] * args.block_dim[1] * args.block_dim[2] # 计算块的数量block_num和高斯数量的阈值num_threshold。

    if args.aabb is None: # 调用get_default_aabb函数计算默认的AABB（轴对齐边界框），并保存到文件中。
    ...
    camera_mask = torch.zeros((len(cameras), block_num), dtype=torch.bool, device=xyz_org.device) # 用于记录每个相机对应的块。
